...
def load_definition_ids(definition_file, include_cn=False):
    ...
    with open(definition_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if definition_id and (True if include_cn else definition_cn):
...
def main():
    TAG = 'cihui'
    # File paths
    definition_file = Path(f'xdhy_core_csv/b_definition_v1.1_{TAG}.csv')
    example_file = Path(f'xdhy_core_csv/c_example_v1.1_{TAG}.csv')
    
    # Load all definition IDs from both files
    print("Loading definition IDs from definition file...")
    definition_ids_exclude_cn = load_definition_ids(definition_file)
    definition_ids = load_definition_ids(definition_file, include_cn=True)
    ...
    missing_in_example = check_missing_ids(definition_ids_exclude_cn, example_definition_ids)
    ...
    if missing_in_example:
        ...
        with open(f'xdhy_core_csv/b_definition_v1.1_{TAG}_missing_ids_in_example.txt', 'w', encoding='utf-8') as f:
            for did in sorted(missing_in_example):
                f.write(f"{did}\n")
    else:
    ...
    if missing_in_definition:
        ...
        for did in sorted(missing_in_definition):
            print(f"  {did}",end=',')
        # Save the missing ids to a file
        with open(f'xdhy_core_csv/b_example_v1.1_{TAG}_missing_ids_in_definition.txt', 'w', encoding='utf-8') as f:
            for did in sorted(missing_in_definition):
                f.write(f"{did}\n")
    else:
