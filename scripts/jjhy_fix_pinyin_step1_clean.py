...
def check_and_correct_pinyin(input_file, output_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        fieldnames = reader.fieldnames + ['cleaned_pinyin']
        ...
        for row in reader:
            ...
            if cleaned_pinyin != original_pinyin:
                row['cleaned_pinyin'] = cleaned_pinyin
                corrections.append({
                    'term': row.get('term', ''),
                ...
                row['cleaned_pinyin'] = original_pinyin
    ...
    with open(output_file, 'w', encoding='utf-8', newline='') as file:
        if not fieldnames:
            fieldnames = ['term', 'pinyin', 'cleaned_pinyin']
    ...
    if corrections:
        ...
        for corr in corrections:
            term_display = f"Term: {corr['term']}" if corr['term'] else ""
            if term_display:
                print(term_display)
    ...
    if issues:
        ...
        print("-" * 80)
        for issue in issues:
            term_display = f"Term: {issue['term']}" if issue['term'] else ""
            if term_display:
                print(term_display)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--default', action='store_true', 
                       help='Use default paths (jjhy_csv/core_pinyin/jjhy_pinyin_all.csv -> jjhy_csv/core_pinyin/jjhy_pinyin_cleaned.csv)')
    
    args = parser.parse_args()
    
    if args.default or (not args.input_file and not args.output_file):
        input_file = 'jjhy_csv/core_pinyin/jjhy_pinyin_all.csv'
        output_file = 'jjhy_csv/core_pinyin/jjhy_pinyin_cleaned.csv'
    else:
