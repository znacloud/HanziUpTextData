...
def fix_pinyin_spacing(input_file, output_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        fieldnames = reader.fieldnames + ['updated_pinyin', 'status']
        line_count = 0
        for row in reader:
            line_count += 1
            ...
            if original_pinyin:
                ...
                context_term = row.get('term')
                new_pinyin, status = split_pinyin(line_count, original_pinyin, context_term)
                if status == "success":
                    success_count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--default', action='store_true', 
                       help='Use default paths (jjhy_csv/core_pinyin/jjhy_pinyin_cleaned.csv -> jjhy_csv/core_pinyin/jjhy_pinyin_spaced.csv)')
    
    args = parser.parse_args()
    
    if args.default or (not args.input_file and not args.output_file):
        input_file = 'jjhy_csv/core_pinyin/jjhy_pinyin_cleaned.csv'
        output_file = 'jjhy_csv/core_pinyin/jjhy_pinyin_spaced.csv'
    else:
