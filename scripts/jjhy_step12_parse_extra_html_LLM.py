...
load_dotenv()
TAG = 'hanzi'
# True when only parsed those invalid parsed
INVALID_PARSED = ''
# True when only parsed those with subdefs
SUBDEFS = ''
...
def main(file_path, flag_num):
    print(f"开始解析HTML内容...{file_path}, flag_num={flag_num}, sample_size={SAMPLE_SIZE}, invalid_parsed={INVALID_PARSED}, subdefs={SUBDEFS}")
    main_df = read_input_files(file_path, invalid_parsed=INVALID_PARSED, subdefs=SUBDEFS)
    ...
    for idx, row in tqdm(main_df.iterrows(), total=total_size):
        ...
        if (idx + 1) % 100 == 0 or idx == total_size - 1:
            ...
            if not temp_df.empty:
                ...
                file_name = f'jjhy_csv/llm_parsed_csv/jjhy_{TAG}_extra_parsed_{flag_num}_{idx+1}.csv'
...
if __name__ == "__main__":
    ...
    for flag_num in flag_nums:
        file_path = f'jjhy_csv/missed_{TAG}_html_{flag_num}.csv'
